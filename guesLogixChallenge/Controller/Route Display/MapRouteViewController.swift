...
extension RouteViewController:MGLMapViewDelegate {
    ...
    func extractCoordinates()-> [CLLocationCoordinate2D] {
        ...
        for (index,edge) in route.enumerated() {
            if index == 0 {
                let airportFirst = edge.source.data
                let latitude = airportFirst.latitude
                let longitude = airportFirst.longitude
    ...
    func printRoute() -> String {
        var label = ""
        
        for edge in route {
            label = label + "\(edge.source.data.iata)--->\(edge.destination.data.iata) | via: \(edge.label!)\n"
        }
